<template>
  <el-dialog
    ...
    <el-form-item label="" prop="skuCode">
      <el-input v-model="dataForm.skuCode" placeholder=""></el-input>
    </el-form-item>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.skuId) {
            this.$http({
              url: this.$http.adornUrl(`/pms/skuinfo/info/${this.dataForm.skuId}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.spuId = data.skuinfo.spuId
                this.dataForm.skuCode = data.skuinfo.skuCode
                ...
                this.dataForm.weight = data.skuinfo.weight
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/pms/skuinfo/${!this.dataForm.skuId ? 'save' : 'update'}`),
              method: 'post',
